def bfs(adj_list, start_node, target_node):
    visited = set()     #Keeps track of nodes we have already visited so we don't revisit them. Set is used because it automatically avoids duplicates.
    queue = []      #This list will act like a queue (FIFO – First In First Out) for BFS.
    queue.append(start_node)  
    parent={}   #This dictionary keeps track of each node’s parent, to help us find the path later.The start node has no parent.
    parent[start_node]=None #The start node has no parent.
    traversal = []
    path=[]
    while queue:
    ...
    while node!=None:
        ...
        node=parent[node]
    path.reverse()
    return traversal, path  # Return traversal after finishing the entire BFS
...
# Perform BFS
traversal, path = bfs(adj_list, start_node, target_node)
